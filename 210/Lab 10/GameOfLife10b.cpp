...
struct cell
{
  int row; // any +/0/- value
  int col; // any +/0/- value
  bool operator<(const cell& c) const {return (1009 * row + col < 1009 * c.row + c.col) ? true : false;}
};
...
void print()
{
  ...
  for (temp.row = MINROW; temp.row <= MAXROW; temp.row++)
  {
    for (temp.col = MINCOL; temp.col <= MAXCOL; temp.col++)
      if (grid.count(temp)) //using count() function in place of containsKey() since it isn't a multilayer map
        cout << grid[temp];
      else
        cout << ' ';
    cout << endl;
  ...
  for (temp.row = MINROW; temp.row <= MAXROW; temp.row++)
  {
    for (temp.col = MINCOL; temp.col <= MAXCOL; temp.col++)
    {
      ...
      switch (neighborCount(temp.row, temp.col))
      {
        ...
        case 2:
          if (grid.count(temp)) newGrid[temp] = 'X';
          break;
